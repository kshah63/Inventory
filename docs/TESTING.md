...
| # | Do | Expect |
| --- | --- | --- |
| 2.1 | Lands on **/browse** | Both rooms' availability per item; search + category chips work |
| 2.2 | **My activity** | Only Priya's checkouts/returns, grouped by day |
| 2.3 | **Requests** → New request (existing item + a free-text one) | Both listed as *Open*; cancel one — it disappears |
| 2.4 | Type `/admin` in the URL bar | Bounced back to /browse — staff can't see admin screens |
| 2.5 | Settings → change password (via /admin/settings is admin-only; staff change is on login… use "Send test"?) — *skip*: staff password resets are done by the super admin on the Users screen | — |
...
| 3.11 | **Requests**: move Priya's request Open → Ordered with a note | Priya's /requests shows the new status + note; WhatsApp update arrives |
